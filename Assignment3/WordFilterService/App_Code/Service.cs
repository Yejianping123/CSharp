...
public class Service : IService
{
    ...
    public string getWebContentAfterFiltering(string URL)
    {
        ...
        if (isURL)
        {
            ...
            string contentCopy = @contentFromURL;

            /*
             * Regex for HTML, replace it with a empty string
             */
            string htmlREGEX = @"(</?)[^>]+?(/?>)|[^a-zA-Z]";
            string filteredOutput = Regex.Replace(contentCopy, htmlREGEX, " ").ToLower();
    ...
    /*
     * Implementation of Word Filter service
     */
    public string WordFilter(string stringToParse)
    {
        string filteredContent = getWebContentAfterFiltering(stringToParse);

        /*
         * Retrieve only unique words afer word filter
         */ 
        HashSet<string> uniqueWords = new HashSet<string>();
        foreach (string s in filteredContent.Split())
        {
            if (s.Trim().Length > 1)
            {
                uniqueWords.Add(s);
        ...
        /*
         * Combine the values stored in the set into a string
         */
        string result = string.Join(" ", uniqueWords.ToArray());
        return result;
    }
}
